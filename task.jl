...
function task1a(xs,ys,l)
   ...
   savefig(string(save_folder,"/comp_lapplace_series_heatmap.pdf"))
   # display(p_all)
end
...
function task1c(ϕ₀)
   ...
   savefig(string( save_folder,"/broken_SOR_heatmap.pdf"))

   plot(broke_SOR[3],title="Maximal error vs step", label="SOR for α = 2.0")
   xlabel!("Timesteps")
   ylabel!("Max error ϵ")
   savefig(string( save_folder,"/broken_SOR_error.pdf"))
